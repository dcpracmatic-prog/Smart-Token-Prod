...
class NativeTarpit:
    ...
    def __init__(self, base_key: bytes, tarpit_mode: str = "cpu", tarpit_seconds: float = 1.5):
        if _LIB is None:
            raise NativeUnavailableError(
                "libfriction.so no encontrado. Compila con `cd cpp && make all` "
                "o usa el backend Python (smart_token_prod.core.SequentialTarpit)."
            )
        if len(base_key) != 32:
        ...
        # Recovery ratchet lives in Python (C++ lib has no recovery fields yet)
        self._recovery_tier = 0
        self._work_factor = 1
        self._cumulative_iters = 0
    ...
    def register_failure(self) -> Dict[str, Any]:
        ...
        snap = self.snapshot()
        # snapshot already includes current recovery_*; apply_failure uses fail_count
        snap = apply_failure_to_snapshot(snap)
        self._recovery_tier = int(snap["recovery_tier"])
        self._work_factor = int(snap["work_factor"])
        self._cumulative_iters = int(snap.get("cumulative_iters", 0) or 0)
        snap["recovery_tier"] = self._recovery_tier
        snap["work_factor"] = self._work_factor
        snap["cumulative_iters"] = self._cumulative_iters
        return snap
    ...
    def reset(self) -> None:
        ...
        self._work_factor = 1
        self._cumulative_iters = 0
        self._fail_count_override = None
        self._tarpit_override = None

    def restore_snapshot(self, snap: Dict[str, Any]) -> None:
        """Rehydrate flags without re-running CPU tarpit; overlay recovery fields."""
        self.reset()
        fc = int(snap.get("fail_count", 0) or 0)
        # Replay at most 2 failures (warning + mutate) — avoid tarpit CPU burn
        for _ in range(min(fc, 2)):
            _LIB.st_tarpit_register_failure(self._handle)
        self._fail_count_override = fc if fc > 2 else None
        self._tarpit_override = True if fc >= 3 else None
        self._recovery_tier = int(snap.get("recovery_tier", 0) or 0)
        wf = snap.get("work_factor", snap.get("recovery_debt", 1))
        self._work_factor = int(wf) if wf not in (None, 0) else 1
        self._cumulative_iters = int(snap.get("cumulative_iters", 0) or 0)
    ...
    def snapshot(self) -> Dict[str, Any]:
        ...
        fc = snap.fail_count
        tarpit = bool(snap.tarpit_triggered)
        if getattr(self, "_fail_count_override", None) is not None:
            fc = self._fail_count_override
        if getattr(self, "_tarpit_override", None):
            tarpit = True
        return {
            "fail_count": fc,
            "flag_fibonacci": bool(snap.flag_fibonacci) or fc >= 1,
            "flag_persistencia": bool(snap.flag_persistencia) or fc >= 2,
            "tarpit_triggered": tarpit,
            "fib_seed": snap.fib_seed,
            "recovery_tier": getattr(self, "_recovery_tier", 0),
            "work_factor": getattr(self, "_work_factor", 1),
            "cumulative_iters": getattr(self, "_cumulative_iters", 0),
        }
